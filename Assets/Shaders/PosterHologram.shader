...
Shader "Silent/ParallaxPoster"
{
	...
	SubShader
	{
		...
		float2 ParallaxOffset(float height, float3 viewDir, float scale, float2 uvs = 0)
		{
			return ( ( height - 1 ) * ( viewDir.xy / viewDir.z ) * scale ) + uvs;
		}


		float2 getUVs(float3 viewDir, float height, float scale, float4 scaleOffset, float2 uv)
		{
			float2 offset = ParallaxOffset(height, viewDir, scale);
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 uv0 = ScaleOffsetCentred( ParallaxOffset(_Layer0Height, viewDir, _ParallaxScale, texcoord0), _Layer0_ST );
			...
			#if !defined(_SUNDISK_SIMPLE)
			float2 uv6 = getUVs( viewDir, _Layer6Height, _ParallaxScale, _Layer6_ST, texcoord0);
			float2 uv5 = getUVs( viewDir, _Layer5Height, _ParallaxScale, _Layer5_ST, texcoord0);
			float2 uv4 = getUVs( viewDir, _Layer4Height, _ParallaxScale, _Layer4_ST, texcoord0);
			#endif
			#if !defined(_SUNDISK_NONE) 
			float2 uv3 = getUVs( viewDir, _Layer3Height, _ParallaxScale, _Layer3_ST, texcoord0);
			float2 uv2 = getUVs( viewDir, _Layer2Height, _ParallaxScale, _Layer2_ST, texcoord0);
			#endif
			float2 uv1 = getUVs( viewDir, _Layer1Height, _ParallaxScale, _Layer1_ST, texcoord0);
